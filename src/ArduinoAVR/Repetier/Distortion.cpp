...
// SB
            ...
            if (isExternalRadiusPoint(ix, iy)) {
				setMatrix(floor(DISTORTION_LIMIT_TO * Printer::axisStepsPerMM[Z_AXIS]), matrixIndex(ix, iy));
				continue;
			}
#endif
// SB
    ...
    Printer::finishProbing();
	// SB - Print Matrix csv format before to extrapolate out pointer.
	printMatrixCsv( true );
