...
// SB // for skeinforge 40 and later, steps to pull the plastic 1 mm inside the
// SB // extruder, not out.  Overridden if EEPROM activated.
...
// SB // 60 is AD8494, AD8495, AD8496 or AD8497 (5mV/degC and 1/4 the price of AD595
// SB // but only MSOT_08 package) 61 is AD8494, AD8495, AD8496 or AD8497 (5mV/degC
// SB // and 1.25 Vref offset like adafruit breakout) 97 Generic thermistor table 1 98
// SB // Generic thermistor table 2 99 Generic thermistor table 3 100 is AD595 101 is
// SB // MAX6675
...
// SB // For Inverting Stepper Enable Pins (Active Low) use 0, Non Inverting (Active
// SB // High) use 1
...
// SB - 1 = PID Temperature control. Is better but needs good PID values. Defaults are
// SB a good start for most extruder. Overridden if EEPROM activated.
// SB */
// SB #define EXT1_HEAT_MANAGER 1
// SB /** Wait x seconds, after reaching target temperature. Only used for M109.
// SB  * Overridden if EEPROM activated. */
...
// SB 
// SB The I state should converge to the exact heater output needed for the target
// SB temperature. To prevent a long deviation from the target zone, this value limits
// SB the lower value. A good start is 30 lower then the optimal value. You need to
// SB leave room for cooling. Overridden if EEPROM activated.
// SB */
...
// SB // maximum time the heater is can be switched on. Max = 255.  Overridden if
// SB // EEPROM activated.
// SB #define EXT1_PID_MAX 255
// SB /** \brief Faktor for the advance algorithm. 0 disables the algorithm.
// SB Overridden if EEPROM activated. K is the factor for the quadratic term, which is
// SB normally disabled in newer versions. If you want to use the quadratic factor
// SB make sure ENABLE_QUADRATIC_ADVANCE is defined. L is the linear factor and seems
// SB to be working better then the quadratic dependency.
// SB */
...
// SB /** The extruder cooler is a fan to cool the extruder when it is heating. If you
// SB  * turn the etxruder on, the fan goes on. */
// SB #define EXT1_EXTRUDER_COOLER_PIN -1
// SB // SB /** PWM speed for the cooler fan. 0=off 255=full speed */
// SB #define EXT1_EXTRUDER_COOLER_SPEED 255
// SB /** Time in ms between a heater action and test of success. Must be more then
// SB  * time between turning heater on and first temp. rise! 0 will disable
// SB  * decoupling test */
// SB #define EXT1_DECOUPLE_TEST_PERIOD 18000
// SB /** Pin which toggles regularly during extrusion allowing jam control. -1 =
// SB  * disabled */
...
/** SB : Google Sheet Repetier-Firmware_Function_Calculator TEMP_GAIN_1 **/
/** SB : TEMP_GAIN	                                                    **/
/** SB : gain	0.936                                                   **/
/** SB : bias	-2                                                      **/
...
// SB #define HEATED_BED_MAX_TEMP 115
...
// SB #define HEATED_BED_SENSOR_TYPE 1
...
*/
// SB #define HEATED_BED_HEAT_MANAGER 1
